...
/**
 * Navigator Static Tabs Page Object Model
 * Handles expand/collapse functionality for static tabs: Definition, Status, Legends, Contacts
 */
export class NavigatorStaticTabs {
    private page: Page;
    ...
    // Navigate to static tabs
    async navigateToDefinitionsTab() {
        await this.page.waitForTimeout(1800);
        const definitionsTab = this.page.getByRole('button', { name: 'Definitions' });
        await definitionsTab.waitFor({ state: 'visible', timeout: 20000 });
        await definitionsTab.click();
        await this.page.waitForTimeout(2000);
        console.log('✓ Navigated to Definitions tab');
    }

    async navigateToStatusTab() {
        await this.page.waitForTimeout(1800);
        const statusTab = this.page.getByRole('button', { name: 'Status' });
    ...
    async navigateToLegendsTab() {
        await this.page.waitForTimeout(1800);
        const legendsTab = this.page.getByRole('button', { name: 'Legends' });
        await legendsTab.waitFor({ state: 'visible', timeout: 20000 });
        await legendsTab.click();
        await this.page.waitForTimeout(2000);
        console.log('✓ Navigated to Legends tab');
    }
    ...
    // Check if Expand All button exists on the current tab
    ...
    async clickExpandAllIfPresent(): Promise<boolean> {
        ...
        if (isPresent) {
            ...
            console.log('ℹ Expand All button not present - no expandable content on this tab');
            return false;
        }
    }
    ...
    // Verify content is expanded (check for toggle elements or expanded rows)
    async verifyContentExpanded() {
        // Wait a bit for expansion to complete
        await this.page.waitForTimeout(1000);
        
        // Check for toggle elements in expanded state
        ...
        if (count > 0) {
            ...
            // Alternative: check for expanded table rows or visible content
    ...
    // Verify tab content is loaded (generic check)
    async verifyTabContentLoaded(tabName: string) {
        await this.page.waitForTimeout(1500);
        
        // Check for table or content container
        ...
        if (hasTable || hasContent) {
            console.log(`✓ ${tabName} tab content loaded successfully`);
        } else {
            console.log(`⚠ ${tabName} tab content may not be fully loaded`);
        }
    }
}
